...
{
	// AnimNotify 可能在角色 Pipeline Tick 之间到达；这里只合并事件，不直接修改 RuntimeData。
	// 重复 Notify 没有额外语义，下一次活动参数阶段只消费一次。
...
void FTurnBackPhaseProcessor::Process(FRuntimeData& RuntimeData, float DeltaTime)
{
	...
	{
		// 只有活动 TurnBack 才能消费事件；bCanYaw 表示输入接管许可，
		// bSecondSegment 是 MotionDriver/AnimBP 选择 d1 的路由标记。
