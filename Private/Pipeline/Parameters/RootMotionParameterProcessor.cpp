 ...
 * @brief 根运动数据提取处理器实现
 *
 * 从 GGYGOAnimInstance 读取当前帧的根骨骼位移（RootMotionDelta）。
 * GGYGOAnimInstance 在 NativeUpdateAnimation 中从 AnimSequence 内置的
 * 根骨骼轨道提取每帧位移，本处理器直接读取该值写入 RuntimeData。
 *
 * 对比旧方案（Speed 曲线）：
 *   旧：FBX → Python 脚本 → AnimSequence.Speed 曲线 → GetCurveValue("Speed")
 *   新：AnimSequence 内置 RM → GGYGOAnimInstance 提取 → 读 RootMotionDelta
 *
 * 新方案优势：
